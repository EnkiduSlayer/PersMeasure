...
{
	cv::rectangle(img, cv::Point(0, 0), cv::Point(img.cols - 1, NN - 1), cv::Scalar(0), FILLED);
	cv::rectangle(img, cv::Point(0, 0), cv::Point(NN - 1, img.rows - 1), cv::Scalar(0), FILLED);
	cv::rectangle(img, cv::Point(0, img.rows - NN), cv::Point(img.cols - 1, img.rows - 1), cv::Scalar(0), FILLED);
	cv::rectangle(img, cv::Point(img.cols - NN, 0), cv::Point(img.cols - 1, img.rows - 1), cv::Scalar(0), FILLED);
}
...
void LSWMS::drawLSegs(cv::Mat &img, std::vector<LSEG> &lSegs, std::vector<double> &errors, int thickness)
{
	...
	colors.push_back(Scalar(200, 0, 0));
	colors.push_back(Scalar(150, 0, 0));
	colors.push_back(Scalar(50, 0, 0));
